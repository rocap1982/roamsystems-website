...
import { gridProducts, type HomeGridProduct } from './homeData';

function Card({ product, i }: { product: HomeGridProduct; i: number }) {
  const [added, setAdded] = useState(false);

  const onAdd = (e: React.MouseEvent) => {
    e.preventDefault();
    window.roamCart?.add(product.cart);
    setAdded(true);
  ...
  return (
    ...
    >
      <a href={product.href} className="relative aspect-[4/3] w-full overflow-hidden bg-[color:var(--color-ink-800)] block">
        <img
          src={product.image}
          alt={product.name}
        ...
        <div className="absolute left-4 top-4 flex items-center gap-2 text-[10px] font-bold uppercase tracking-widest text-white drop-shadow">
          ...
          <span>{String(i + 1).padStart(2, '0')} / {String(gridProducts.length).padStart(2, '0')}</span>
        </div>
      ...
      <div className="flex flex-1 flex-col p-6">
        <a href={product.href}>
          <h3 className="font-black uppercase leading-tight tracking-tight text-white text-lg transition-colors group-hover:text-[color:var(--color-amber-base)]">
            {product.name}
          </h3>
        </a>
        <p className="mt-2 flex-1 text-sm text-[color:var(--color-ink-300)]">{product.blurb}</p>
        <div className="mt-5 flex items-end justify-between">
          <div>
            <div className="text-[10px] font-bold uppercase tracking-widest text-[color:var(--color-ink-400)]">From</div>
        ...
        <button
          type="button"
          onClick={onAdd}
          className={`group/btn mt-5 inline-flex items-center justify-between gap-2 rounded-md px-5 py-3 text-xs font-black uppercase tracking-wider transition-all ${
            added
              ? 'bg-green-600 text-white'
              : 'bg-[color:var(--color-amber-base)] text-black hover:bg-[color:var(--color-amber-bright)]'
          }`}
        >
    ...
    <section id="products" className="relative bg-[color:var(--color-ink-800)] py-24 md:py-32">
      <div className="mx-auto max-w-7xl px-4 md:px-8">
        ...
        >
          <div>
            <div className="flex items-center gap-3">
              ...
              <span className="text-xs font-black uppercase tracking-[0.2em] text-[color:var(--color-amber-base)]">The Range</span>
            </div>
        ...
        <div className="mt-14 text-center">
          <a
            href="/products"
            className="inline-flex items-center gap-3 rounded-md border border-[color:var(--color-ink-500)] bg-[color:var(--color-ink-900)] px-8 py-4 text-sm font-black uppercase tracking-wider text-white transition-all hover:border-[color:var(--color-amber-base)] hover:text-[color:var(--color-amber-base)]"
          >
            View All Products →
          </a>
        </div>
